(function() {
  ...
  a.register("comp.layer.outlogin", function(j) {
    ...
    v.setQRcode = function() {
      if (window.SINA_QRCODE_LOGIN) {
        ...
        D.set({
          ...
          domain: g.sso.domain,
          qrcode_size: 128
        }).register("qrcode_timeout", function(E) {
          i = false;
          v.getQRcode()
        }).register("qrcode_exception", function(E) {
          i = false;
          v.getQRcode()
        }).register("login_success", function(E) {
          j.setStyle(t.qrcode_tip, "display", "none");
          f.fire("login_success")
        }).register("login_failure", function(E) {
          f.fire("login_failure")
        }).register("qrcode_scanned", function(E) {
    ...
    v.updateLayer = function() {
      v.insert();
      j.foreach(g.styles, function(G, F) {
    ...
    v.getUserUid = function() {
      window.sinaSSOController.prelogin({}, function(D) {
    ...
    l.getSinaCookie = function() {
      var F = j.cookie.get("SUP");
      var E = j.cookie.get("ALF");
      var D = j.cookie.get("SUR");
      if (!F && !E) {
        return null
      }
      var G = F || D;
    ...
    l.destroy = function() {
      ...
      j.foreach(["mask", "SSO", "plugin", "suggestor"], function(D) {
        var F = v[D];
